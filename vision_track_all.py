        ...
        if cap_flag == 1:
            ...
            while (cap.isOpened()):
                ...
                if position_fps_flag:
                    ...
                    position_fps_title.place(relx=0.781, rely=0.9)
                    position_fps = tkinter.Label(window, font=("Microsoft YaHei", 11, "bold"), text="")
                    position_fps.place(relx=0.81, rely=0.9)
            ...
            if show_track_online.get():
                ...
                while (cap.isOpened()):
                    ...
                    if show_track_3.get():
                        ...
                        if position_time_dect_flag:
                            position_time_title_dect = tkinter.Label(window, font=("Microsoft YaHei", 11, "bold"),
                                                                     text="")
                            position_time_title_dect.place(relx=0.781, rely=0.87)
                            position_time_dect = tkinter.Label(window, font=("Microsoft YaHei", 11, "bold"), text="")
                            position_time_dect.place(relx=0.81, rely=0.87)
                    ...
                    if have_target_count.count(1) >= 5 and trans_track_first:
                        ...
                        if siamcar_choose.get():
                            tracker_siamcar.init(img_track, init_rect) # 跟踪器初始化
                        if siammask_choose.get():
                            tracker_siammask.init(img_track, init_rect) # 跟踪器初始化
                        if siamrpnpp_choose.get():
                            tracker_siamrpnpp.init(img_track, init_rect)  # 跟踪器初始化
                    ...
                    if track_flag == 1:
                        ...
                        if siamcar_choose.get():
                            ...
                            bbox_siamcar = list(map(int, outputs_siamcar['bbox']))
                            cv2.rectangle(search_img, (bbox_siamcar[0], bbox_siamcar[1]),
                                          (bbox_siamcar[0] + bbox_siamcar[2],
                                           bbox_siamcar[1] + bbox_siamcar[3]),
                                          (0, 0, 255), 3)
                            ...
                            # 计算文本位置，右上角位置减去文本大小
                            text_x = bbox_siamcar[0] + bbox_siamcar[2] - text_size[0]
                            text_y = bbox_siamcar[1] - text_size[1]

                            # 在图像上绘制文本
                            cv2.putText(search_img, text, (text_x, text_y), font, font_scale, font_color,
                                        font_thickness)


                            # 添加目标轨迹点
                            track_points_siamcar.append((bbox_siamcar[0] + bbox_siamcar[2] // 2,
                                                         bbox_siamcar[1] + bbox_siamcar[3] // 2))
                            if len(track_points_siamcar) > 80:  # 轨迹长度
                                track_points_siamcar.pop(0)
                        ...
                        if siammask_choose.get():
                            ...
                            # 计算文本位置，右上角位置减去文本大小
                            text_x = bbox_mask[0] + bbox_mask[2] - text_size[0]
                            text_y = bbox_mask[1] - text_size[1]
                            ...
                            # 添加目标轨迹点
                            track_points_siammask.append(
                                (bbox_mask[0] + bbox_mask[2] // 2, bbox_mask[1] + bbox_mask[3] // 2))
                            if len(track_points_siammask) > 80:  # 轨迹长度
                                track_points_siammask.pop(0)
                        ...
                        if siamrpnpp_choose.get():
                            ...
                            # 计算文本位置，右上角位置减去文本大小
                            text_x = bbox_rpnpp[0] + bbox_rpnpp[2] - text_size[0]
                            text_y = bbox_rpnpp[1] - text_size[1]
                            ...
                            # 添加目标轨迹点
                            track_points_siamrpnpp.append(
                                (bbox_rpnpp[0] + bbox_rpnpp[2] // 2, bbox_rpnpp[1] + bbox_rpnpp[3] // 2))
                            if len(track_points_siamrpnpp) > 80:  # 轨迹长度
                                track_points_siamrpnpp.pop(0)
...
if __name__ == '__main__':
    ...
    # 跟踪模型选择
    label_frame_track_txt = tk.Label(window, text="跟踪器选择", font=("Microsoft YaHei", 15, "bold"))
    label_frame_track_txt.place(relx=0.805, rely=0.56)
    label_frame_track = tk.LabelFrame(window, text="", font=("Microsoft YaHei", 15, "bold"))
    label_frame_track.place(relx=0.773, rely=0.6, relwidth=0.15, relheight=0.05)
    window.update()

    # 模块2
    label_frame_txt = tk.Label(window, text="目标位置", font=("Microsoft YaHei", 15, "bold"))
    label_frame_txt.place(relx=0.81, rely=0.66)
    label_frame = tk.LabelFrame(window, text="", font=("Microsoft YaHei", 15, "bold"))
    label_frame.place(relx=0.773, rely=0.70, relwidth=0.15, relheight=0.1)

    # 模块3
    label_frame_txt = tk.Label(window, text="其他数据", font=("Microsoft YaHei", 15, "bold"))
    label_frame_txt.place(relx=0.81, rely=0.81)
    label_frame = tk.LabelFrame(window, text="", font=("Microsoft YaHei", 15, "bold"))
    label_frame.place(relx=0.773, rely=0.85, relwidth=0.15, relheight=0.1)
    ...
    # 跟踪器siamcar
    siamcar_choose = tk.BooleanVar() # "显示轨迹"
    siamcar_choose.set(False)  # 默认勾选
    # 创建一个勾选按钮
    siamcar_choose_button = tk.Checkbutton(window, text="CAR", variable=siamcar_choose,fg="red")
    # 将勾选按钮放置在合适的位置
    siamcar_choose_button.config(width=5, height=1)
    siamcar_choose_button.place(relx=0.775, rely=0.61)

    # 跟踪器siammask
    siammask_choose = tk.BooleanVar()  # "显示轨迹"
    siammask_choose.set(False)  # 默认勾选
    # 创建一个勾选按钮
    siammask_choose_button = tk.Checkbutton(window, text="MASK", variable=siammask_choose,fg="green")
    # 将勾选按钮放置在合适的位置
    siammask_choose_button.config(width=5, height=1)
    siammask_choose_button.place(relx=0.82, rely=0.61)

    # 跟踪器siamrpn++
    siamrpnpp_choose = tk.BooleanVar()  # "显示轨迹"
    siamrpnpp_choose.set(False)  # 默认勾选
    # 创建一个勾选按钮
    siamrpnpp_choose_button = tk.Checkbutton(window, text="RPN++", variable=siamrpnpp_choose,fg="blue")
    # 将勾选按钮放置在合适的位置
    siamrpnpp_choose_button.config(width=5, height=1)
    siamrpnpp_choose_button.place(relx=0.87, rely=0.61)
